...
{
    public partial class frmMetas : Form
    {
        public frmMetas()
        {
            InitializeComponent();
            dtpMetas.Format = DateTimePickerFormat.Custom;
            dtpMetas.CustomFormat = "MM/yyyy";
            PreparaVentana();
        }        
        ...
        {       
            dgvMetas.DefaultCellStyle.Font = controlEvents.RowFont;
        }
        ...
        public void Archivo(object oFilePath)
        {
            ...
            try{
                ...
                string[] Layout;
                Layout = new string[] { "No Empleado", "Clave De Gestor", "Nombre Completo", "Subdirector", "Gerente", "Coordinador", "Supervisor", "No Empleado Supervisor", "Cartera", "Segmento", "Puesto", "Sucursal", "Fecha de Ingreso a la Cartera", "Observaciones", "Calidad", "Promesas", "Cumplimiento", "01_05", "06_12", "13_19", "20_26", "27_31", "TOTAL", "X", "XX", "XXX" };
            ...
            sErrorFechas = validaFechas(tblMetas);
            if (sErrorFechas != ""){
            ...
            Invoke((Action)delegate(){
                dgvMetas.DataSource = tblMetas;                
                dgvMetas.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.DisplayedCells);
                picWait.Visible = false;
                btnCargar.Visible = btnArchivo.Visible = dgvMetas.Visible = true;
            });

            Mensaje("", Color.DimGray, lblMensajes);
        }

        private string validaFechas(DataTable tblMetas)
        {
            DateTime dtValidador = new DateTime();
            //int i = 0;
            string Dato = "";
            try{
                foreach (DataRow drPagos in tblMetas.Rows){
                    if (drPagos.Table.Columns.Contains("Fecha de Ingreso a la Cartera")){
                        if (!DateTime.TryParse(drPagos["Fecha de Ingreso a la Cartera"].ToString(), out dtValidador))
                        {
                            Dato = dtValidador.ToString();
                            if (Dato != "01/01/0001 12:00:00 a. m."){
                                sCuentaError = drPagos["No. Empleado."].ToString();
                                return "Falló, verificar la columna Fecha de Ingreso a la Cartera del No. Empleado. " + drPagos["No. Empleado."].ToString() + " contiene un tipo de dato no válido.";
                            }
                        }
                    }
                }
        ...
        {
            DataTable PagosError = new DataTable(), CreaTemporalPag = new DataTable(), tblMetas = new DataTable();
            //string sErrorFechas = "";
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'Meta_" + Ejecutivo.Datos["idEjecutivo"] + "'  AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.Meta_" + Ejecutivo.Datos["idEjecutivo"]);

            if (!DataBaseConn.Execute("DropTemporalMetas")){
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.Meta_" + Ejecutivo.Datos["idEjecutivo"] + " ( ";
            for (int i = 0; i < tblMetas.Columns.Count; i++){
                if (tblMetas.Columns[i].ColumnName.Contains("Fecha"))
                    sCreateTable += ("\r\n [" + tblMetas.Columns[i] + "] DATETIME NULL, ");
                else if (tblMetas.Columns[i].ColumnName.Contains("Entrada") || tblMetas.Columns[i].ColumnName.Contains("Salida") || tblMetas.Columns[i].ColumnName.Contains("InicioDescanso") || tblMetas.Columns[i].ColumnName.Contains("FinDescanso"))
                    sCreateTable += ("\r\n [" + tblMetas.Columns[i] + "] DATETIME NULL, ");                
                else
                    sCreateTable += ("\r\n [" + tblMetas.Columns[i] + "] VARCHAR(MAX) NULL, ");
            }
            ...
            if (!DataBaseConn.Fill(CreaTemporalPag, "CreaTemporalMetas")){
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.Meta_" + Ejecutivo.Datos["idEjecutivo"];
                ...
                string Fecha = dtpMetas.Value.ToString("yyyy-MM-dd");

                Mensaje("Insertando Intentos, por favor espere...", Color.DimGray, lblMensajes);
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.Estadistica.[3.2.Metas] @Fecha, @idEjecutivo ");
                ...
                if (!DataBaseConn.Fill(tblVal,"InsertaMetas")){
                ...
                //    lector.Close();
                //    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; picWaitBulk.Visible = false; dgvMetas.DataSource = null; txtRuta.Text = ""; lblInstrucciones.Text = ""; lblRegistros.Text = ""; });
                //    return;
                //}
                //else
                //{
                //    Mensaje("Las metas del mes seleccionado se insertarón en el sistema.", Color.DimGray, lblMensajes);
                //}
                lector.Close();

                Mensaje("Las metas se insertarón en el sistema.", Color.DimGray, lblMensajes);
        ...
        {                    
            Mensaje("Espere unos segundos...", Color.DimGray, lblInstrucciones);
