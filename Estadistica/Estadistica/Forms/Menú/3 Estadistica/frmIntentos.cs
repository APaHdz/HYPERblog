...
{
    public partial class frmIntentos : Form
    {
        public frmIntentos()
        {
            InitializeComponent();
            //dtpIntentos.Format = DateTimePickerFormat.Custom;
            //dtpIntentos.CustomFormat = "MM/yyyy";
            PreparaVentana();
        }        

        #region Variables
        int iNumberRecords;
        DataTable tblIntIntentos = new DataTable();
        dbFileReader lector;
        string sCuentaError = "";
        int idCartera = 0;
        string Servidor = "";
        ...
        public void PreparaVentana()
        {
            ...
            cmbServidor.DataSource = Catálogo.ValoresDelCatálogo(33);
            

            dgvIntentos.DefaultCellStyle.Font = controlEvents.RowFont;
        }
        ...
        public void Archivo(object oFilePath)
        {
            ...
            try{
                ...
                tblIntIntentos.Load(lector.Reader);                

                if (tblIntIntentos.Rows.Count == 0){
                ...
                Layout = new string[] { "list_id", "campaign_id", "call_date", "status", "phone_number", "user" };
                
                foreach (string sColumna in Layout)
                    if (!tblIntIntentos.Columns.Contains(sColumna)){
            ...
            sErrorFechas = validaFechas(tblIntIntentos);
            if (sErrorFechas != ""){
            ...
            Mensaje("Archivo con " + iNumberRecords.ToString("#,0") + " registros y " + tblIntIntentos.Columns.Count.ToString("#,0") + " columnas.", Color.DimGray, lblRegistros);
            Mensaje("Verifique la información y presione Cargar.", Color.SlateGray, lblInstrucciones);


            Invoke((Action)delegate(){
                dgvIntentos.DataSource = tblIntIntentos;                
                dgvIntentos.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.DisplayedCells);
                picWait.Visible = false;
                btnCargar.Visible = btnArchivo.Visible = dgvIntentos.Visible = true;
            });

            Mensaje("", Color.DimGray, lblMensajes);
        }

        private string validaFechas(DataTable tblIntIntentos)
        {
            DateTime dtValidador = new DateTime();
            //int i = 0;
            string Dato = "";
            try{
                foreach (DataRow drPagos in tblIntIntentos.Rows){
        ...
        public void CargaInteIntentos(object oDatos)
        {
            DataTable PagosError = new DataTable(), CreaTemporalPag = new DataTable(), tblIntIntentos = new DataTable();
            //string sErrorFechas = "";
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'Inte_" + Ejecutivo.Datos["idEjecutivo"] + "' AND schema_id=" + EsquemaTemporal + " ) " +
                " DROP TABLE dbEstadistica.Temp.Inte_" + Ejecutivo.Datos["idEjecutivo"]);

            if (!DataBaseConn.Execute("DropTemporalInteIntentos")){
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.Inte_" + Ejecutivo.Datos["idEjecutivo"] + " ( ";
            for (int i = 0; i < tblIntIntentos.Columns.Count; i++){
                if (tblIntIntentos.Columns[i].ColumnName.Contains("date"))
                    sCreateTable += ("\r\n [" + tblIntIntentos.Columns[i] + "] DATETIME NULL, ");
                else if (tblIntIntentos.Columns[i].ColumnName.Contains("Entrada") || tblIntIntentos.Columns[i].ColumnName.Contains("Salida") || tblIntIntentos.Columns[i].ColumnName.Contains("InicioDescanso") || tblIntIntentos.Columns[i].ColumnName.Contains("FinDescanso"))
                    sCreateTable += ("\r\n [" + tblIntIntentos.Columns[i] + "] DATETIME NULL, ");                
                else
                    sCreateTable += ("\r\n [" + tblIntIntentos.Columns[i] + "] VARCHAR(50) NULL, ");
            }
            ...
            if (!DataBaseConn.Fill(CreaTemporalPag, "CreaTemporalIntentos")){
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.Inte_" + Ejecutivo.Datos["idEjecutivo"];
                        blkData.BulkCopyTimeout = 30 * 60;
                        blkData.WriteToServer(tblIntIntentos);
                    }
                    catch (Exception ex){
                ...
                string Fecha = dtpIntentos.Value.ToString("yyyy-MM-dd");

                Mensaje("Insertando Intentos, por favor espere...", Color.DimGray, lblMensajes);
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.Estadistica.[3.1.Intentos] @Fecha, @idEjecutivo, @idCartera, @Servidor");
                ...
                DataBaseConn.CommandParameters.AddWithValue("@Servidor", Servidor);

                DataTable tblVal = new DataTable();

                if (!DataBaseConn.Fill(tblVal,"InsertaIntentos")){
                    Mensaje("Falló al insertar Intentos.", Color.Red, lblMensajes);
                    lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; });
                    return;
                }

                if (tblVal.Columns.Contains("Validación"))
                {
                    string sMensaje = lector.CreateTab("xValidacion_" + DateTime.Now.ToString("yyMMdd_hhmmss"), tblVal);
                    if (sMensaje != "")
                        Mensaje("Existe un error al guardar los registros incorrectos, no se subió la información en el sistema.", Color.Red, lblMensajes);

                    Mensaje("Se encontrarón registros incorrectos.", Color.Red, lblMensajes);
                    lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; picWaitBulk.Visible = false; dgvIntentos.DataSource = null; txtRuta.Text = ""; lblInstrucciones.Text = ""; lblRegistros.Text = ""; });
                    return;
                }
                else
                {
                    Mensaje("Los intentos del día seleccionado se insertarón en el sistema.", Color.DimGray, lblMensajes);
                }
                lector.Close();

                Mensaje("Los intentos se insertarón en el sistema.", Color.DimGray, lblMensajes);
        ...
        private void btnCargar_Click(object sender, EventArgs e)
        {
            ...
            DataBaseConn.StartThread(CargaInteIntentos);
        }

        private void frmInteIntentos_FormClosed(object sender, FormClosedEventArgs e)
        {
