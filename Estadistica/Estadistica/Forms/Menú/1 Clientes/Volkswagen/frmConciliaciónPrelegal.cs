...
{
    public partial class frmConciliaciónPrelegal : Form
    {
        public frmConciliaciónPrelegal()
        {
            InitializeComponent();
        ...
        #region Variables
        int iNumberRecords;
        DataTable tblConciliacionPrelegal = new DataTable();
        dbFileReader lector;
        string sCuentaError = "";
        #endregion
        ...
        {           
            dgvConciliacionPrelegal.DefaultCellStyle.Font = controlEvents.RowFont;
        }
        ...
        public void Archivo(object oFilePath)
        {
            ...
            tblConciliacionPrelegal = new DataTable();
            ...
            try{
                ...
                tblConciliacionPrelegal.Load(lector.Reader);

                if (tblConciliacionPrelegal.Rows.Count == 0)
                {
                ...
                foreach (string sColumna in Layout)
                    if (!tblConciliacionPrelegal.Columns.Contains(sColumna))
                    {
            ...
            sErrorFechas = validaFechas(tblConciliacionPrelegal);
            if (sErrorFechas != ""){
            ...
            Mensaje("Archivo con " + iNumberRecords.ToString("#,0") + " registros y " + tblConciliacionPrelegal.Columns.Count.ToString("#,0") + " columnas.", Color.DimGray, lblRegistros);
            Mensaje("Verifique la información y presione Cargar.", Color.SlateGray, lblInstrucciones);


            Invoke((Action)delegate(){
                dgvConciliacionPrelegal.DataSource = tblConciliacionPrelegal;                
                dgvConciliacionPrelegal.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.DisplayedCells);
                picWait.Visible = false;
                btnCargar.Visible = btnArchivo.Visible = dgvConciliacionPrelegal.Visible = true;
            });

            Mensaje("", Color.DimGray, lblMensajes);
        }

        private string validaFechas(DataTable tblConciliacionPrelegal)
        {
            DateTime dtValidador = new DateTime();
            //int i = 0;
            string Dato = "";
            try{
                foreach (DataRow drConciliacion in tblConciliacionPrelegal.Rows)
                {                    
                    if (drConciliacion.Table.Columns.Contains("FECHA DE ALTA"))
                    {
                        if (!DateTime.TryParse(drConciliacion["FECHA DE ALTA"].ToString(), out dtValidador))
                        {
                            Dato = dtValidador.ToString();
                            if (Dato != "01/01/0001 12:00:00 a. m."){
                                sCuentaError = drConciliacion["CLIENTE"].ToString();
                                return "Falló, verificar la columna FECHA DE ALTA del cliente " + drConciliacion["CLIENTE"].ToString() + " contiene un tipo de dato no válido.";
                            }
                        }
                    }

                    if (drConciliacion.Table.Columns.Contains("FECHA ASIGNACIÓN AGENCIA"))
                    {
                        if (!DateTime.TryParse(drConciliacion["FECHA ASIGNACIÓN AGENCIA"].ToString(), out dtValidador))
                        {
                            Dato = dtValidador.ToString();
                            if (Dato != "01/01/0001 12:00:00 a. m.")
                            {
                                sCuentaError = drConciliacion["CLIENTE"].ToString();
                                return "Falló, verificar la columna FECHA ASIGNACIÓN AGENCIA del cliente " + drConciliacion["CLIENTE"].ToString() + " contiene un tipo de dato no válido.";
                            }
                        }
                    }
        ...
        public void CargaConciliacionPrelegal(object oDatos)
        {
            //DataTable PagosError = new DataTable(), CreaTemporalPag = new DataTable(), tblConciliacionPrelegal = new DataTable();
            DataTable tblConciliacionPrelegal = new DataTable();
            ...
            for (int i = 0; i < tblConciliacionPrelegal.Columns.Count; i++)
            {
                if (tblConciliacionPrelegal.Columns[i].ColumnName.Contains("FECHA"))
                    sCreateTable += ("\r\n [" + tblConciliacionPrelegal.Columns[i] + "] DATE NULL, ");
                else
                    sCreateTable += ("\r\n [" + tblConciliacionPrelegal.Columns[i] + "] VARCHAR(8000) NULL, ");
            }
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.WriteToServer(tblConciliacionPrelegal);
                    }
                    catch (Exception ex){
                ...
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.[Volkswagen].[2.1.Conciliacion_Prelegal] @idEjecutivo, @Fecha");
                ...
                if (!DataBaseConn.Execute("InsertaConciliacionPrelegal"))
                {
                    Mensaje("Falló al insertar conciliación Prelegal.", Color.Red, lblMensajes);
                    lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; });
                    return;
                }
                ...
                Invoke((Action)delegate{
                    ...
                    dgvConciliacionPrelegal.DataSource = null;
        ...
        private void btnCargar_Click(object sender, EventArgs e)
        {
            ...
            DataBaseConn.StartThread(CargaConciliacionPrelegal);
        }
