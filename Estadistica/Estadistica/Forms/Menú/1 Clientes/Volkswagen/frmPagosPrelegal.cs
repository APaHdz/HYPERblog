...
{
    public partial class frmPagosPrelegal : Form
    {
        public frmPagosPrelegal()
        {
            InitializeComponent();
            dtpMesPagosPrelegal.Format = DateTimePickerFormat.Custom;
            dtpMesPagosPrelegal.CustomFormat = "MM/yyyy";
            PreparaVentana();
        }        

        #region Variables
        int iNumberRecords;
        DataTable tblPagosPrelegal = new DataTable();
        dbFileReader lector;
        string sCuentaError = "";
        #endregion
        ...
        {           
            dgvPagosPrelegal.DefaultCellStyle.Font = controlEvents.RowFont;
        }
        ...
        public void Archivo(object oFilePath)
        {
            ...
            tblPagosPrelegal = new DataTable();
            ...
            try{
                ...
                tblPagosPrelegal.Load(lector.Reader);

                if (tblPagosPrelegal.Rows.Count == 0)
                {
                ...
                foreach (string sColumna in Layout)
                    if (!tblPagosPrelegal.Columns.Contains(sColumna))
                    {
            ...
            sErrorFechas = validaFechas(tblPagosPrelegal);
            if (sErrorFechas != ""){
            ...
            Mensaje("Archivo con " + iNumberRecords.ToString("#,0") + " registros y " + tblPagosPrelegal.Columns.Count.ToString("#,0") + " columnas.", Color.DimGray, lblRegistros);
            Mensaje("Verifique la información y presione Cargar.", Color.SlateGray, lblInstrucciones);


            Invoke((Action)delegate(){
                dgvPagosPrelegal.DataSource = tblPagosPrelegal;                
                dgvPagosPrelegal.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.DisplayedCells);
                picWait.Visible = false;
                btnCargar.Visible = btnArchivo.Visible = dgvPagosPrelegal.Visible = true;
            });
        ...
        private string validaFechas(DataTable tblConciliacionPrelegal)
        {
            ...
            string Dato = "";
            try{
                foreach (DataRow drConciliacion in tblPagosPrelegal.Rows)
                {
                    //Fecha
        ...
        public void CargaPagosPrelegal(object oDatos)
        {            
            DataTable tblPagosPrelegal = new DataTable();
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'PagP_" + Ejecutivo.Datos["idEjecutivo"] + "'  AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.PagP_" + Ejecutivo.Datos["idEjecutivo"]);

            if (!DataBaseConn.Execute("DropTemporalPagP")){
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.PagP_" + Ejecutivo.Datos["idEjecutivo"] + " ( ";
            for (int i = 0; i < tblPagosPrelegal.Columns.Count; i++)
            {
                if (tblPagosPrelegal.Columns[i].ColumnName == "Fecha")
                    sCreateTable += ("\r\n [" + tblPagosPrelegal.Columns[i] + "] DATE NULL, ");
                else
                    sCreateTable += ("\r\n [" + tblPagosPrelegal.Columns[i] + "] VARCHAR(8000) NULL, ");
            }
            ...
            if (!DataBaseConn.Fill(CreaTemporalPag, "CreaTemporalPagP")){
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.PagP_" + Ejecutivo.Datos["idEjecutivo"];
                        blkData.BulkCopyTimeout = 30 * 60;
                        blkData.WriteToServer(tblPagosPrelegal);
                    }
                    catch (Exception ex){
                ...
                string Fecha = dtpMesPagosPrelegal.Value.ToString("yyyy-MM-dd");

                Mensaje("Insertando pagos Prelegal, por favor espere...", Color.DimGray, lblMensajes);
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.[Volkswagen].[2.2.Pagos_Prelegal] @idEjecutivo, @Fecha");
                ...
                if (!DataBaseConn.Execute("InsertaPagosPrelegal"))
                {
                    Mensaje("Falló al insertar pagos Prelegal.", Color.Red, lblMensajes);
                    lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; });
                    return;
                }
                ...
                Invoke((Action)delegate{
                    ...
                    dgvPagosPrelegal.DataSource = null;
        ...
        private void btnCargar_Click(object sender, EventArgs e)
        {
            ...
            DataBaseConn.StartThread(CargaPagosPrelegal);
        }
