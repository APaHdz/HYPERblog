...
{
    public partial class frmCargaInbound : Form
    {
        public frmCargaInbound()
        {
            InitializeComponent();
        ...
        #region Variables
        int iNumberRecords;
        DataTable tblCargaInbound = new DataTable();
        dbFileReader lector;
        string sCuentaError = "";
        #endregion
        ...
        {           
            dgvInbound.DefaultCellStyle.Font = controlEvents.RowFont;
        }
        ...
        public void Archivo(object oFilePath)
        {
            ...
            try{
                ...
                tblCargaInbound.Load(lector.Reader);

                if (tblCargaInbound.Rows.Count == 0)
                {
                ...
                for (int i = 0; i < tblCargaInbound.Columns.Count; i++)
                    tblCargaInbound.Columns[i].ColumnName = tblCargaInbound.Columns[i].ColumnName.Replace(" ", "").Replace(".", "").Replace("/", "").Replace("*", "").Replace("(", "").Replace(")", "").Replace("_", "").Replace("$", "").Replace("¿", "").Replace("?", "").Replace("#", "").ToUpper();

                string[] Layout;
                Layout = new string[] { "calldate", "clid", "src", "dst", "dcontext", "channel", "dstchannel", "user", "lastapp", "lastdata", "duration", "billsec", "disposition", "amaflags", "accountcode", "uniqueid", "userfield" };
                
                foreach (string sColumna in Layout)
                    if (!tblCargaInbound.Columns.Contains(sColumna.Replace(" ", "").Replace(".", "").Replace("/", "").Replace("*", "").Replace("(", "").Replace(")", "").Replace("_", "").Replace("$", "").Replace("¿", "").Replace("?", "")))
                    {
            ...
            sErrorFechas = validaFechas(tblCargaInbound);
            if (sErrorFechas != ""){
            ...
            Mensaje("Archivo con " + iNumberRecords.ToString("#,0") + " registros y " + tblCargaInbound.Columns.Count.ToString("#,0") + " columnas.", Color.DimGray, lblRegistros);
            Mensaje("Verifique la información y presione Cargar.", Color.SlateGray, lblInstrucciones);


            Invoke((Action)delegate(){
                dgvInbound.DataSource = tblCargaInbound;                
                dgvInbound.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.DisplayedCells);
                picWait.Visible = false;
                btnCargar.Visible = btnArchivo.Visible = dgvInbound.Visible = true;
            });
        ...
        private string validaFechas(DataTable tblConciliacionLeasing)
        {
            ...
            string Dato = "";
            try{
                foreach (DataRow drGestion in tblCargaInbound.Rows)
                {
                    //Fecha
                    if (drGestion.Table.Columns.Contains("CALLDATE"))
                    {
                        if (!DateTime.TryParse(drGestion["CALLDATE"].ToString(), out dtValidador))
                        {
                            Dato = dtValidador.ToString();
                            if (Dato != "01/01/0001 12:00:00 a. m."){
                                sCuentaError = drGestion["UNIQUEID"].ToString();
                                return "Falló, verificar la columna CALLDATE del contrato " + drGestion["UNIQUEID"].ToString() + " contiene un tipo de dato no válido.";
                            }
                        }               
                    }                    
                }
        ...
        {            
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'DaIn'  AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.DaIn");

            if (!DataBaseConn.Execute("DropTemporalDaIn")){
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.DaIn ( ";
            for (int i = 0; i < tblCargaInbound.Columns.Count; i++)
            {
                if (tblCargaInbound.Columns[i].ColumnName == "CALLDATE")
                    sCreateTable += ("\r\n [" + tblCargaInbound.Columns[i] + "] DATE NULL, ");
                //else if (tblCargaInbound.Columns[i].ColumnName.Contains("HORAINICIO"))
                //    sCreateTable += ("\r\n [" + tblCargaInbound.Columns[i] + "] DATETIME NULL, ");
                //else if (tblCargaInbound.Columns[i].ColumnName.Contains("MONTOSEGUIMIENTO"))
                //    sCreateTable += ("\r\n [" + tblCargaInbound.Columns[i] + "] MONEY NULL, ");                
                else
                    sCreateTable += ("\r\n [" + tblCargaInbound.Columns[i] + "] VARCHAR(MAX) NULL, ");
            }
            ...
            if (!DataBaseConn.Fill(CreaTemporalPag, "CreaTemporalDaIn")){
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.DaIn";
                        blkData.BulkCopyTimeout = 30 * 60;
                        blkData.WriteToServer(tblCargaInbound);
                    }
                    catch (Exception ex){
                ...
                string Fecha = dtpFechaInbound.Value.ToString("yyyy-MM-dd");

                Mensaje("Insertando cuentas únicas, por favor espere...", Color.DimGray, lblMensajes);
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.[Daimler].[1.3.Inbound] @idEjecutivo, @Fecha");
                ...
                if (!DataBaseConn.Execute("InsertaDaimlerInbound"))
                {
                    Mensaje("Falló al insertar inbound Daimler.", Color.Red, lblMensajes);
                    lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; });
                    return;
                }
