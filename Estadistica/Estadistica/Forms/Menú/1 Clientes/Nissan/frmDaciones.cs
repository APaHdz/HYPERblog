...
{
    public partial class frmDaciones : Form
    {
        public frmDaciones()
        {
            InitializeComponent();
            dtpDaciones.Format = DateTimePickerFormat.Custom;
            dtpDaciones.CustomFormat = "MM/yyyy";
            PreparaVentana();
        }        

        #region Variables
        int iNumberRecords;
        DataTable tblDaciones = new DataTable();
        dbFileReader lector;
        string sCuentaError = "";
        #endregion
        ...
        {           
            dgvDaciones.DefaultCellStyle.Font = controlEvents.RowFont;
        }
        ...
        public void Archivo(object oFilePath)
        {
            ...
            try{
                ...
                tblDaciones.Load(lector.Reader);

                if (tblDaciones.Rows.Count == 0)
                {
                ...
                for (int i = 0; i < tblDaciones.Columns.Count; i++)
                    tblDaciones.Columns[i].ColumnName = tblDaciones.Columns[i].ColumnName.Replace(" ", "").Replace(".", "").Replace("/", "").Replace("*", "").Replace("(", "").Replace(")", "").Replace("_", "").Replace("$", "").Replace("¿", "").Replace("?", "").Replace("#", "").ToUpper();

                string[] Layout;
                Layout = new string[] { "Contrato","Recuperado","Despacho", "Localidad", "Valor", "Saldo" };
                
                foreach (string sColumna in Layout)
                    if (!tblDaciones.Columns.Contains(sColumna.Replace(" ", "").Replace(".", "").Replace("/", "").Replace("*", "").Replace("(", "").Replace(")", "").Replace("_", "").Replace("$", "").Replace("¿", "").Replace("?", "")))
                    {
            ...
            sErrorFechas = validaFechas(tblDaciones);
            if (sErrorFechas != ""){
            ...
            Mensaje("Archivo con " + iNumberRecords.ToString("#,0") + " registros y " + tblDaciones.Columns.Count.ToString("#,0") + " columnas.", Color.DimGray, lblRegistros);
            Mensaje("Verifique la información y presione Cargar.", Color.SlateGray, lblInstrucciones);


            Invoke((Action)delegate(){
                dgvDaciones.DataSource = tblDaciones;                
                dgvDaciones.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.DisplayedCells);
                picWait.Visible = false;
                btnCargar.Visible = btnArchivo.Visible = dgvDaciones.Visible = true;
            });
        ...
        private string validaFechas(DataTable tblConciliacionLeasing)
        {
            ...
            string Dato = "";
            try{
                foreach (DataRow drConciliacion in tblDaciones.Rows)
                {
                    //Fecha
        ...
        public void CargaDaciones(object oDatos)
        {            
            DataTable tblDaciones = new DataTable();
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'Daci'  AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.Daci");

            if (!DataBaseConn.Execute("DropTemporalDaci"))
            {
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.Daci ( ";
            for (int i = 0; i < tblDaciones.Columns.Count; i++)
            {
                if (tblDaciones.Columns[i].ColumnName == "RECUPERADO")
                    sCreateTable += ("\r\n [" + tblDaciones.Columns[i] + "] DATE NULL, ");
                else if (tblDaciones.Columns[i].ColumnName.Contains("HORAAMPM)"))
                    sCreateTable += ("\r\n [" + tblDaciones.Columns[i] + "] DATETIME NULL, ");
                else if (tblDaciones.Columns[i].ColumnName.Contains("IMPORTE"))
                    sCreateTable += ("\r\n [" + tblDaciones.Columns[i] + "] MONEY NULL, ");                
                else
                    sCreateTable += ("\r\n [" + tblDaciones.Columns[i] + "] VARCHAR(MAX) NULL, ");
            }
            ...
            if (!DataBaseConn.Fill(CreaTemporalPag, "CreaTemporalDaci"))
            {
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.Daci";
                        blkData.BulkCopyTimeout = 30 * 60;
                        blkData.WriteToServer(tblDaciones);
                    }
                    catch (Exception ex){
                ...
                string Fecha = dtpDaciones.Value.ToString("yyyy-MM-dd");

                Mensaje("Insertando centros de acopio del mes, por favor espere...", Color.DimGray, lblMensajes);
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.[Nissan].[1.2.Daciones] @idEjecutivo, @Fecha");
                ...
                if (!DataBaseConn.Execute("InsertaDaciones"))
                {
                    Mensaje("Falló al insertar daciones.", Color.Red, lblMensajes);
                    lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; });
                    return;
                }
        ...
        private void btnCargar_Click(object sender, EventArgs e)
        {
            ...
            DataBaseConn.StartThread(CargaDaciones);
        }
