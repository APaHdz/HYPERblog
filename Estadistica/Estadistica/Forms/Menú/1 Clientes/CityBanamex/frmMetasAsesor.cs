...
{
    public partial class frmMetasAsesor : Form
    {
        public frmMetasAsesor()
        {
            InitializeComponent();
        ...
        {           
            dgvMetasAsesor.DefaultCellStyle.Font = controlEvents.RowFont;
        }
        ...
        public void Archivo(object oFilePath)
        {
            ...
            try{
                ...
                //    tblMetas.Columns[i].ColumnName = tblMetas.Columns[i].ColumnName.Replace(" ", "").Replace(".", "").Replace("/", "").Replace("*", "").Replace("(", "").Replace(")", "").Replace("-", "").Replace("_", "").Replace("$", "").Replace("¿", "").Replace("?", "").Replace("#", "").ToUpper();

                string[] Layout;                
                Layout = new string[] { "AGENCIA", "ASESOR", "PUESTO", "SUPERVISOR", "COORDINADOR", "TIPO DE RMT", "ETAPA", "PORTAFOLIO", "META RMTS LEVANTADOS", "ALCANCE A META DE RMTS LEVANTADOS", "META EN RMTS BOOKEADOS", "ALCANCE A META DE RMTS BOOKEADOS", "META ($)", "ALCANCE A META ($)", "ANTIGÜEDAD", "ESTATUS", "RFC" };
                
                foreach (string sColumna in Layout)
                    if (!tblMetas.Columns.Contains(sColumna.Trim())){
            ...
            Invoke((Action)delegate(){
                dgvMetasAsesor.DataSource = tblMetas;                
                dgvMetasAsesor.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.DisplayedCells);
                picWait.Visible = false;
                btnCargar.Visible = btnArchivo.Visible = dgvMetasAsesor.Visible = true;
            });
        ...
        private string validaFechas(DataTable tblMetas)
        {
            ...
            string Dato = "";
            try{
                foreach (DataRow drNegociaciones in tblMetas.Rows){
        ...
        {
            DataTable SummaryError = new DataTable(), CreaTemporalPag = new DataTable(), tblMetas = new DataTable();
            //string sErrorFechas = "";
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'Metas' AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.Metas");
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.Metas ( ";
            for (int i = 0; i < tblMetas.Columns.Count; i++){
                if (tblMetas.Columns[i].ColumnName == "Fecha de recepcion" || tblMetas.Columns[i].ColumnName == "Fecha de Vencimiento")
                    sCreateTable += ("\r\n [" + tblMetas.Columns[i] + "] DATE NULL, ");
                else if (tblMetas.Columns[i].ColumnName == "Saldo Negociado" || tblMetas.Columns[i].ColumnName == "Importe Pago inicial")
                    sCreateTable += ("\r\n [" + tblMetas.Columns[i] + "] MONEY NULL, ");
                else if (tblMetas.Columns[i].ColumnName == "Num de Pagos")
                    sCreateTable += ("\r\n [" + tblMetas.Columns[i] + "] SMALLINT NULL, ");
                else
                    sCreateTable += ("\r\n [" + tblMetas.Columns[i] + "] VARCHAR(300) NULL, ");
            }
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.Metas";
                ...
                string Fecha = dtpMetasAsesor.Value.ToString("yyyy-MM-dd");

                Mensaje("Insertando metas, por favor espere...", Color.DimGray, lblMensajes);
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.CityBanamex.[2.5.Metas_Asesor] @idEjecutivo, @Fecha");
                ...
                if (!DataBaseConn.Execute("InsertaMetas")){
                    Mensaje("Falló al insertar metas por asesor CityBanamex.", Color.Red, lblMensajes);
                    lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; });
                    return;
                }
                ...
                Invoke((Action)delegate{
                    ...
                    dgvMetasAsesor.DataSource = null;
