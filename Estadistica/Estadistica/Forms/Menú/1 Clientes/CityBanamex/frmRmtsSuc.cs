...
{
    public partial class frmRmtsSuc : Form
    {
        public frmRmtsSuc()
        {
            InitializeComponent();
        ...
        {           
            dgvRMTSSUC.DefaultCellStyle.Font = controlEvents.RowFont;
        }
        ...
        public void Archivo(object oFilePath)
        {
            ...
            try{
                ...
                //string[] Layout;                
                //Layout = new string[] { "Nombre", "Supervisor", "4TO CORTE", "Settlement (pt)", "Settlement a Plazo (cp)", "Payment Plan (lp)", "Saldo Negociado" };
            ...
            Invoke((Action)delegate(){
                dgvRMTSSUC.DataSource = tblNegociaciones;                
                dgvRMTSSUC.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.DisplayedCells);
                picWait.Visible = false;
                btnCargar.Visible = btnArchivo.Visible = dgvRMTSSUC.Visible = true;
            });
        ...
        public void CargaNegociaciones(object oDatos)
        {
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                ...
                string Fecha = dtpRMTS.Value.ToString("yyyy-MM-dd");

                Mensaje("Insertando cierre corte, por favor espere...", Color.DimGray, lblMensajes);
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.CityBanamex.[2.4.RMTS_SUC] @idEjecutivo, @Fecha");
                ...
                if (!DataBaseConn.Execute("InsertaRMTSSuc")){
                    Mensaje("Falló al insertar RMTS sucursales CityBanamex.", Color.Red, lblMensajes);
                    lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; });
                    return;
                }
