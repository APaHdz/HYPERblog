    ...
    public partial class frmAsistencia : Form
    {
        ...
        public void Archivo(object oFilePath)
        {
            ...
            try{
                ...
                tblAsistencia.Load(lector.Reader);

                if (tblAsistencia.Rows.Count == 0)
                {
                ...
                //    if (!tblAsistencia.Columns.Contains(sColumna))
                //    {
            ...
            Mensaje("Archivo con " + iNumberRecords.ToString("#,0") + " registros y " + tblAsistencia.Columns.Count.ToString("#,0") + " columnas.", Color.DimGray, lblRegistros);
            Mensaje("Verifique la información y presione Cargar.", Color.SlateGray, lblInstrucciones);


            Invoke((Action)delegate(){
                dgvAsistencia.DataSource = tblAsistencia;                
        ...
        {            
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'Asis_" + Ejecutivo.Datos["idEjecutivo"] + "'  AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.Asis_" + Ejecutivo.Datos["idEjecutivo"]);

            if (!DataBaseConn.Execute("DropTemporalAsis")){
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.Asis_" + Ejecutivo.Datos["idEjecutivo"] + " ( ";
            for (int i = 0; i < tblAsistencia.Columns.Count; i++)
            {
                if (tblAsistencia.Columns[i].ColumnName == "Fecha")
                    sCreateTable += ("\r\n [" + tblAsistencia.Columns[i] + "] DATE NULL, ");
                else
                    sCreateTable += ("\r\n [" + tblAsistencia.Columns[i] + "] VARCHAR(100) NULL, ");
            }
            ...
            if (!DataBaseConn.Fill(CreaTemporalAsi, "CreaTemporalAsis")){
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.Asis_" + Ejecutivo.Datos["idEjecutivo"];
                        blkData.BulkCopyTimeout = 30 * 60;
                        blkData.WriteToServer(tblAsistencia);
                    }
                    catch (Exception ex){
                ...
                string Fecha = dtpMesAsistencia.Value.ToString("yyyy-MM-dd");
                int Dia = dtpMesAsistencia.Value.Day;

                Mensaje("Asistencia en proceso, por favor espere...", Color.DimGray, lblMensajes);


                if (chkSucursales.Checked == false)
                {
                    DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.[CityBanamex].[6.1.Asistencia]  @Fecha, @Dia, @idEjecutivo ");
                    DataBaseConn.CommandParameters.AddWithValue("@Fecha", Fecha);
                    DataBaseConn.CommandParameters.AddWithValue("@Dia", Dia);
                    DataBaseConn.CommandParameters.AddWithValue("@idEjecutivo", Ejecutivo.Datos["idEjecutivo"]);
                }
                else
                {
                    DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.[CityBanamex].[6.1.AsistenciaSucursal]  @Fecha, @Dia, @idEjecutivo ");
                    DataBaseConn.CommandParameters.AddWithValue("@Fecha", Fecha);
                    DataBaseConn.CommandParameters.AddWithValue("@Dia", Dia);
                    DataBaseConn.CommandParameters.AddWithValue("@idEjecutivo", Ejecutivo.Datos["idEjecutivo"]);
                }
                ...
                DataSet tblReporteAsistencia = new DataSet();

                if (!DataBaseConn.Fill(tblReporteAsistencia, "ExcelAsistencia"))
                {
                    TerminaBúsqueda("Falló la generación del reporte.", true);
                    return;
                }

                if (tblReporteAsistencia.Tables[0].Columns.Contains("Mensaje"))
                {
                    Mensaje(tblReporteAsistencia.Tables[0].Rows[0]["Mensaje"].ToString(), Color.Crimson, lblMensajes);
                ...
                int Pestañas = tblReporteAsistencia.Tables.Count;
                string Nombre = "";
                for(int i=0; i<Pestañas;i++)
                {
                    if (i == 0)
                        Nombre = "Total Ejecutivos";
                    else
                        Nombre = (i < 10 ? "0" + i.ToString() : i.ToString());

                    tblReporteAsistencia.Tables[i].TableName = Nombre;
                }

                string sRutaExcel = "";

                if (tblReporteAsistencia.Tables[0].Rows.Count == 0)
                {
                ...
                if (sRutaExcel == "")
                {
                    tblReporteAsistencia.Dispose();
                    return;
                }

                string sResultado = ExcelXML.ExportToExcelSAX(ref tblReporteAsistencia, sRutaExcel);
