...
{
    public partial class frmFallabelaGestiones : Form
    {
        public frmFallabelaGestiones()
        {
            InitializeComponent();
        ...
        {           
            dgvDiaGestiones.DefaultCellStyle.Font = controlEvents.RowFont;
        }
        ...
        public void Archivo(object oFilePath)
        {
            ...
            DataColumn Id_cuenta = new DataColumn("Id_cuenta");
            DataColumn Producto = new DataColumn("Producto");
            DataColumn Clavegesttel = new DataColumn("Clavegesttel");
            DataColumn Fecha = new DataColumn("Fecha");
            DataColumn Hora_inicio = new DataColumn("Hora_inicio");
            DataColumn Hora_fin = new DataColumn("Hora_fin");
            DataColumn Telefono = new DataColumn("Telefono");
            DataColumn Resultado = new DataColumn("Resultado");
            DataColumn Monto_negociado = new DataColumn("Monto_negociado");
            DataColumn Fecha_promesa = new DataColumn("Fecha_promesa");
            DataColumn Comentario = new DataColumn("Comentario");
            DataColumn Causa = new DataColumn("Causa");
            workTable.Columns.Add(Id_cuenta);
            workTable.Columns.Add(Producto);
            workTable.Columns.Add(Clavegesttel);
            workTable.Columns.Add(Fecha);
            workTable.Columns.Add(Hora_inicio);
            workTable.Columns.Add(Hora_fin);
            workTable.Columns.Add(Telefono);
            workTable.Columns.Add(Resultado);
            workTable.Columns.Add(Monto_negociado);
            workTable.Columns.Add(Fecha_promesa);
            workTable.Columns.Add(Comentario);
            workTable.Columns.Add(Causa);
            ...
            using (StreamReader ReaderObject = new StreamReader(FileToRead))
            {
                ...
                {                    
                    try
                    {
                        ...
                        row1["Id_cuenta"] = valores[0].ToString();
                        row1["Producto"] = valores[1].ToString();
                        row1["Clavegesttel"] = valores[2].ToString();
                        row1["Fecha"] = valores[3].ToString();
                        row1["Hora_inicio"] = valores[4].ToString();
                        row1["Hora_fin"] = valores[5].ToString();
                        row1["Telefono"] = valores[6].ToString();
                        row1["Resultado"] = valores[7].ToString();
                        row1["Monto_negociado"] = valores[8].ToString();
                        row1["Fecha_promesa"] = valores[9].ToString();
                        row1["Comentario"] = valores[10].ToString();
                        row1["Causa"] = valores[11].ToString();
            ...
            Invoke((Action)delegate(){
                dgvDiaGestiones.DataSource = workTable;                
                dgvDiaGestiones.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.DisplayedCells);
                picWait.Visible = false;
                btnCargar.Visible = btnArchivo.Visible = dgvDiaGestiones.Visible = true;
            });
        ...
        public void CargaRemesa(object oDatos)
        {
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'GesSF' AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.GesSF");
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.GesSF ( ";
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.GesSF";
                ...
                string Fecha = dtpDiaGestiones.Value.ToString("yyyy-MM-dd");

                Mensaje("Insertando gestiones, por favor espere...", Color.DimGray, lblMensajes);
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.Soriana.[4.1.GestionesFalabella] @idEjecutivo, @Fecha");
                ...
                if (!DataBaseConn.Execute("InsertaGestionesFalabella")){
                    Mensaje("Falló al insertar gestiones Falabella.", Color.Red, lblMensajes);
                    //lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; });
                    return;
                }

                Mensaje("Las gestiones se insertarón en el sistema.", Color.DimGray, lblMensajes);
                ...
                Invoke((Action)delegate{
                    ...
                    dgvDiaGestiones.DataSource = null;
