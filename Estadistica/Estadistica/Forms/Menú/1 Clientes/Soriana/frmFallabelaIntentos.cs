...
{
    public partial class frmFallabelaIntentos : Form
    {
        public frmFallabelaIntentos()
        {
            InitializeComponent();
        ...
        {           
            dgvDiaIntentos.DefaultCellStyle.Font = controlEvents.RowFont;
        }
        ...
        public void Archivo(object oFilePath)
        {
            ...
            DataColumn Hora_insert = new DataColumn("Hora_insert");
            DataColumn Detalle = new DataColumn("Detalle");
            
            workTable.Columns.Add(idcuenta);
            workTable.Columns.Add(Telefono);
            workTable.Columns.Add(Fecha);
            workTable.Columns.Add(Hora_insert);
            workTable.Columns.Add(Detalle);
            ...
            using (StreamReader ReaderObject = new StreamReader(FileToRead))
            {
                ...
                {                    
                    try
                    {
                        ...
                        row1["Telefono"] = valores[1].ToString();
                        row1["Fecha"] = valores[2].ToString();
                        row1["Hora_insert"] = valores[3].ToString();
                        row1["Detalle"] = valores[4].ToString();
            ...
            Invoke((Action)delegate(){
                dgvDiaIntentos.DataSource = workTable;                
                dgvDiaIntentos.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.DisplayedCells);
                picWait.Visible = false;
                btnCargar.Visible = btnArchivo.Visible = dgvDiaIntentos.Visible = true;
            });
        ...
        public void CargaRemesa(object oDatos)
        {
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'IntSF' AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.IntSF");
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.IntSF ( ";
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.IntSF";
                ...
                string Fecha = dtpDiaIntentos.Value.ToString("yyyy-MM-dd");

                Mensaje("Insertando intentos, por favor espere...", Color.DimGray, lblMensajes);
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.Soriana.[4.2.IntentosFalabella] @idEjecutivo, @Fecha");
                ...
                if (!DataBaseConn.Execute("InsertaIntentosFalabella")){
                    Mensaje("Falló al insertar intentos Falabella.", Color.Red, lblMensajes);
                    //lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; });
                    return;
                }
                ...
                Invoke((Action)delegate{
                    ...
                    dgvDiaIntentos.DataSource = null;
