...
{
    public partial class frmIreneCuentas : Form
    {
        public frmIreneCuentas()
        {
            InitializeComponent();
        ...
        public void Archivo(object oFilePath)
        {
            ...
            try{
                ...
                string[] Layout;
                Layout = new string[] { "F# Gestion", "cliente", "Despacho", "ESTADÍO", "TIPO_DE_LLAMADA", "TIPO_DE_GESTION", "Cuenta", "Contrato (18 Digitos)", "Producto", "Producto", "Ciclo", "Mora", "Nombre_Cliente", "Calle", "Colonia", "CP", "Delegacion_Población", "Entidad", "Tel Casa", "Tel Ofic", "Capturista", "Ejecutivo", "Supervisor_Gerente", "DIVISION", "REGIONAL", "PLAZA", "F#Captura", "Hora (Am/Pm)", "Gestion", "RR", "CR", "CA", "Fech_PP", "Importe ($)", "Causas De No Pago", "CVE_EJEC", "NOMBRE_EJEC", "SUPERVISOR", "PRODUCTO", "SUCURSAL" };
        ...
        {            
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'Ircu'  AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.Ircu");

            if (!DataBaseConn.Execute("DropTemporalIrcu")){
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.Ircu ( ";
            for (int i = 0; i < tblPagosLeasing.Columns.Count; i++)
            {
                if (tblPagosLeasing.Columns[i].ColumnName == "F# Gestion" || tblPagosLeasing.Columns[i].ColumnName == "F#Captura" || tblPagosLeasing.Columns[i].ColumnName == "Fech_PP")
                    sCreateTable += ("\r\n [" + tblPagosLeasing.Columns[i] + "] DATE NULL, ");
                else if (tblPagosLeasing.Columns[i].ColumnName.Contains("Hora (Am/Pm)"))
                    sCreateTable += ("\r\n [" + tblPagosLeasing.Columns[i] + "] DATETIME NULL, ");
                else if (tblPagosLeasing.Columns[i].ColumnName.Contains("Importe ($)"))
                    sCreateTable += ("\r\n [" + tblPagosLeasing.Columns[i] + "] MONEY NULL, ");                
                else
                    sCreateTable += ("\r\n [" + tblPagosLeasing.Columns[i] + "] VARCHAR(MAX) NULL, ");
            }
            ...
            if (!DataBaseConn.Fill(CreaTemporalPag, "CreaTemporalIrcu")){
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.Ircu";
                ...
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.[Bancomer].[1.3.IreneCuentasUnicas] @idEjecutivo, @Fecha");
                ...
                if (!DataBaseConn.Execute("InsertaCuentasUnicas"))
                {
                    Mensaje("Falló al insertar cuentas únicas.", Color.Red, lblMensajes);
                    lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; });
                    return;
                }
