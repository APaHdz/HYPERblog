...
{
    public partial class frmAsignacionTDCGracia : Form
    {
        public frmAsignacionTDCGracia()
        {
            InitializeComponent();
        ...
        public void Archivo(object oFilePath)
        {
            ...
            try{
                ...
                string[] Layout;
                Layout = new string[] { "NO_CUENTA","NO_CUENTA_CYBER","CAMPAÑA","CAMPAÑA_REAL","OBSERVACIONES","ESTATUS_CTA","DIA_CORTE","TOTAL_DEUDOR_ASIG","PRIORIDAD","PAGO_MINIMO_ASIG","FILA_ACT","DMSTATUS","DMREASSG","DMAGENCY","DMAGNREF","TOTAL_DEUDOR_ACT","PAGO_MINIMO_ACT","U6STACTECA","FECHA_ASIGNACION","COD_BLOQUEO","MARCA_AFINITI","MARCA_CYBER_AFI","U6PAGODIF","U6DOMACT2" };
        ...
        public void CargaTDCGracia(object oDatos)
        {
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'TDCGracia'  AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.TDCGracia");

            if (!DataBaseConn.Execute("DropTemporalTDCGracia")){
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.TDCGracia ( ";
            for (int i = 0; i < tblTDCGracia.Columns.Count; i++)
            {
                if (tblTDCGracia.Columns[i].ColumnName == "DMAGNREF" || tblTDCGracia.Columns[i].ColumnName == "FECHA_ASIGNACION")
                    sCreateTable += ("\r\n [" + tblTDCGracia.Columns[i] + "] DATE NULL, ");
                else if (tblTDCGracia.Columns[i].ColumnName == "TOTAL_DEUDOR_ASIG" || tblTDCGracia.Columns[i].ColumnName == "PAGO_MINIMO_ASIG" || tblTDCGracia.Columns[i].ColumnName == "TOTAL_DEUDOR_ACT" || tblTDCGracia.Columns[i].ColumnName == "PAGO_MINIMO_ACT")
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.TDCGracia";
                ...
                string MesAño = dtpMesAño.Value.ToString("yyyy-MM-dd");

                Mensaje("Insertando información, por favor espere...", Color.DimGray, lblMensajes);
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC [Santander].[AsigTDCGracia] @Fecha_Proceso, @idEjecutivo, @MesAño, @ArchivoExtension");
                ...
                DataBaseConn.CommandParameters.AddWithValue("@MesAño", MesAño);
