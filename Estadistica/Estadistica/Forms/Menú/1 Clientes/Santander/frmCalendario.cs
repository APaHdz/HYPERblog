...
namespace Estadistica
{
    public partial class frmCalendario : Form
    {
        public frmCalendario()
        {
            ...
            dtpCalendario.CustomFormat = "MM/yyyy";
            PreparaVentana();
        }        
        ...
        {           
            dgvCalendario.DefaultCellStyle.Font = controlEvents.RowFont;
        }
        ...
        public void Archivo(object oFilePath)
        {
            ...
            try{
                ...
                string[] Layout;
                Layout = new string[] { "Fecha","Asignacion","P1P2","P3P4","DesAsignacion","CongelarPV" };
            ...
            Invoke((Action)delegate(){
                dgvCalendario.DataSource = tblTDCGracia;                
                dgvCalendario.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.DisplayedCells);
                picWait.Visible = false;
                btnCargar.Visible = btnArchivo.Visible = dgvCalendario.Visible = true;
            });
        ...
        public void CargaTDCGracia(object oDatos)
        {
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'TDCGraciaCalendario'  AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.TDCGraciaCalendario");

            if (!DataBaseConn.Execute("DropTemporalTDCGraciaCalendario"))
            {
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.TDCGraciaCalendario ( ";
            for (int i = 0; i < tblTDCGracia.Columns.Count; i++)
            {
                if (tblTDCGracia.Columns[i].ColumnName == "Fecha")
            ...
            if (!DataBaseConn.Fill(CreaTemporalPag, "CreaTemporalTDCGraciaCalendario"))
            {
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.TDCGraciaCalendario";
                ...
                string Fecha = dtpCalendario.Value.ToString("yyyy-MM-dd");

                Mensaje("Insertando información, por favor espere...", Color.DimGray, lblMensajes);
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC [Santander].[TDCGraciaCalendario] @Fecha_Proceso, @idEjecutivo");
