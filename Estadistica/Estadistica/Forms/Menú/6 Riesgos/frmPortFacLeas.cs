...
{
    public partial class frmPortFacLeas : Form
    {
        public frmPortFacLeas()
        {
            InitializeComponent();
            dtpMesPagos.Format = DateTimePickerFormat.Custom;
            dtpMesPagos.CustomFormat = "dd/MM/yyyy";
        ...
        public void Archivo(object oFilePath)
        {
            ...
            try{
                ...
                Layout = new string[] {"EXTERNAL_REFERENCE1", "QUOTATION", "TYPE_OF_CONTRACT", "BUSINESS_PARTNER",
                    " BP_NAME", "BP_CATEGORY", "CUSTOMER_GROUP", "TYPE_OF_CAR", "PRODUCT_GROUP", "PRODUCT_TYPE" ,
                    "PORTFOLIO_TYPE", "VIN", "MODEL", "MODEL_DESC__PRODUCT_ID_", "BRAND", "DEALER", "ADRESS_STATE",
                    "ACTIVATED_CURRENT_MONTH", "ACTIVATION_DATE", "CONTRACT_START_DATE", "START_OF_BILLING_DATE",
                    "APPLICATION_RECEIPT_DATE", "CONTRACT_END_DATE", "ORIGINAL_TERM", "CALCULATED_TERM", "LEGAL_STATUS",
                    "OVERDUE_DAYS", "OVERDUE_CLASS", "OVERDUE_DAYS_MAX", "DEFAULT", "DEFAULT_DD", "OVERDUE_DAYS_DD",
                    "CURRENCY", "CXC", "UNDUE_PRINCIPAL", "UNDUE_INTEREST", "UNDUE_TAX", "UNDUE_RESIDUAL", 
                    "OVERDUE_TOTAL", "OVERDUE_PRINCIPAL", "OVERDUE_INTEREST", "OVERDUE_TAX", "OVERDUE_BUCKET_DEBIT",
                    "OVERDUE_BUCKET_CREDIT", "GROSS_PRICE", "CONTRACTUAL_RESIDUAL_PAYMENT", "RESVALUE_PERCENTAGE", 
                    "FINANCED_AMOUNT", "FINAL_PAYMENT", "DOWN_PAYMENT", "DOWNPAYMENT_PERCENTAGE", "INTEREST_RATE",
                    "CREDIT_CAMPAIGN_ID", "GROWTH_PLAN_ID", "NAFIN","FINANCING_PROD_ID", "FINANCING_PROD_DESCRIPTION",
                    "BORROWER_UNIT", "CND_ID", "CLOSING_DATE", "CALCULATION_DATE"
};
        ...
        public void CargaPagos(object oDatos)
        {
            ...
            DataBaseConn.SetCommand("IF EXISTS (SELECT 1 FROM dbEstadistica.sys.tables WHERE name = 'PORTFACLEAS' AND schema_id=6 ) " +
                " DROP TABLE dbEstadistica.Temp.PORTFACLEAS");

            if (!DataBaseConn.Execute("DropTemporalPortafolio")){
            ...
            string sCreateTable = "CREATE TABLE dbEstadistica.Temp.PORTFACLEAS ( ";
            ...
            using (SqlBulkCopy blkData = DataBaseConn.bulkCopy){{
                    try{
                        ...
                        blkData.DestinationTableName = "dbEstadistica.Temp.PORTFACLEAS";
                ...
                DataBaseConn.SetCommand("WAITFOR DELAY '00:00:00'; EXEC dbEstadistica.Riesgos.[PORTFACLEASING] @idCartera,@idProducto, @idEjecutivo, @Fecha ");                                
                DataBaseConn.CommandParameters.AddWithValue("@idCartera", idCartera);
                DataBaseConn.CommandParameters.AddWithValue("@idProducto", idCartera);
                ...
                if (!DataBaseConn.Execute("InsertaPortafolio")){
                    Mensaje("Falló al insertar portafolio.", Color.Red, lblMensajes);
                    lector.Close();
                    Invoke((Action)delegate() { btnArchivo.Visible = true; picWaitBulk.Visible = false; });
                    return;
                }

                Mensaje("El portafolio de leasing se insertó en el sistema.", Color.DimGray, lblMensajes);                
        ...
        private void cmbCarteras_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmbCarteras.Text != "")
            {
               ...
               // Producto = cmbProducto.Text;
                ...
                txtRuta.Text = "";
                LimpiaMensajes();
                Mensaje("Seleccione un archivo con el layout de Portafolio" + Producto + ".", Color.SlateGray, lblMensajes);
            }
            else
